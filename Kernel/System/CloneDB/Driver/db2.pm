...
package Kernel::System::CloneDB::Driver::db2;
...
Kernel::System::CloneDB::Driver::db2

=head1 SYNOPSIS

CloneDBs db2 Driver delegate
...
sub TablesList {
    ...
    $Param{DBObject}->Prepare(
        SQL => "select tabname from syscat.tables where tabschema = 'DB2INST1'"
    ) || die @!;
...
sub ColumnsList {
    ...
    $Param{DBObject}->Prepare(
        SQL => "SELECT COLNAME
                FROM syscat.columns
                WHERE TABNAME = ? and TABSCHEMA='DB2INST1'",
        Bind => [
            \$Param{Table},
        ],
    ) || die @!;
